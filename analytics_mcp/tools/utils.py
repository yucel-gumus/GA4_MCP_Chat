...
def process_date_ranges(date_ranges: list[dict]) -> list[dict]:
  """Processes date ranges to convert them into the correct format.

  Args:
    date_ranges: A list of date ranges.

  Returns:
    A list of date ranges with the correct format.
  """
  processed_ranges = []
  for date_range in date_ranges:
    ...
    # Convert "last N months" to "N*30daysAgo"
    months_match = re.match(r"(\d+)\s*months?Ago", start_date)
    if months_match:
      num_months = int(months_match.group(1))
      start_date = f"{num_months * 30}daysAgo"

    processed_ranges.append({"start_date": start_date, "end_date": end_date})
